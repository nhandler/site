...
const personalInfoScreenIndex = 0;
const educationScreenIndex = 3;
const experienceScreenIndex = 5;
const eventScreenIndex = 7;
      ...
      {lights.map(([name], i) => (
        <div key={i} className={clsx(styles.lightGroup, isDisabled(i) && styles.disabled)} onClick={() => !isDisabled(i)}>
          {formIndex >= personalInfoScreenIndex && formIndex < educationScreenIndex && i == 0 ? (
            ...
            <img className={styles.activeMole} src={mole2} onClick={() => setFormIndex(educationScreenIndex)} alt="" />
          ) : formIndex >= experienceScreenIndex && formIndex < eventScreenIndex && i === 2 ? (
            <img className={styles.activeMole} src={mole3} onClick={() => setFormIndex(experienceScreenIndex)} alt="" />
          ) : formIndex >= eventScreenIndex && formIndex < finishScreenIndex && i === 3 ? (
            <img className={styles.activeMole} src={mole4} onClick={() => setFormIndex(eventScreenIndex)} alt="" />
          ) : (
            <div>
              {i === 0 ? (
                <img className={styles.hole} src={mole1} onClick={() => setFormIndex(personalInfoScreenIndex)} alt="" />
              ) : i === 1 ? (
                <img className={styles.hole} src={mole2} onClick={() => setFormIndex(educationScreenIndex)} alt="" />
              ) : i === 2 ? (
                <img className={styles.hole} src={mole3} onClick={() => setFormIndex(experienceScreenIndex)} alt="" />
              ) : i === 3 ? (
                <img className={styles.hole} src={mole4}  onClick={() => setFormIndex(eventScreenIndex)} alt="" />
              ) : (
